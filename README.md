...
The tutorial starts with `kind` for the first Kubernetes basics and then uses `k3s` on WSL2 as the default local ML platform. `kind` stays available as the starter and fallback path.

The expansion chapters cover STACKIT first, then a provider-neutral cloud portability track, then CI/CD, the capstone that ties the full workflow together, and a conclusion with future reading. An optional Flyte add-on after the conclusion compares the same workflow against a non-Kubeflow orchestrator. The FAQ covers local reset procedures for the local Kubernetes path and standalone Kubeflow Pipelines.
