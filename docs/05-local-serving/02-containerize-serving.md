...
## Make the Image Available to the Default Cluster

If you are on the default k3s path, verify that Docker has the image. k3s uses Docker as its runtime in this tutorial, so no separate image-load command is needed:
...
- `/healthz` succeeds
- `/predict` succeeds
- the serving image is available to `k3s`, or loaded into `kind` if that is your fallback path
